...
@Slf4j
@Transactional
public class SetMembershipRequestStatusToAccepted implements ExecutionListener {

    private final MembershipRequestRepository membershipRequestRepository;

    @Override
    public void notify(DelegateExecution execution) throws Exception {
        log.info(LogMessages.EXECUTE, SetMembershipRequestStatusToAccepted.class.getName());
        ...
        if (membershipRequestOptional.isPresent()) {
            ...
            membershipRequest.setReviewResult(ReviewResult.ACCEPTED);
            this.membershipRequestRepository.save(membershipRequest);
        }

        log.info(LogMessages.FINISHED, SetMembershipRequestStatusToAccepted.class.getName());
    }
}
